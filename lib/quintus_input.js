...
Q.component("platformerControls2", {
    defaults: {
      speed: 200,
      jumpSpeed: -300,
      collisions: []
    ...
    step: function(dt) {
      ...
      if(p.ignoreControls === undefined || !p.ignoreControls) {
        ...
        // Follow along the current slope, if possible.
        if(p.collisions !== undefined && p.collisions.length > 0 && (Q.inputs['left2'] || Q.inputs['right'] || p.landed > 0)) {
          if(p.collisions.length === 1) {
            collision = p.collisions[0];
          } else {
            // If there's more than one possible slope, follow slope with negative Y normal
        ...
        if(Q.inputs['left2'] && !p.jumping && p.landed > 0) {
          p.direction = 'left2';
          if(collision && p.landed > 0) {
            p.vx = p.speed * collision.normalY;
            p.vy = -p.speed * collision.normalX;
        ...
        } else if(Q.inputs['right2'] && !p.jumping && p.landed > 0) {
          p.direction = 'right2';
          if(collision && p.landed > 0) {
            p.vx = -p.speed * collision.normalY;
            p.vy = p.speed * collision.normalX;
        ...
        if(p.landed > 0 && (Q.inputs['up2'] || Q.inputs['action']) && !p.jumping) {
          p.vy = p.jumpSpeed;
          p.landed = -dt;
          p.jumping = true;
        } else if(Q.inputs['up2'] || Q.inputs['action']) {
          this.entity.trigger('jump', this.entity);
          p.jumping = true;
        }

        if(p.jumping && !(Q.inputs['up2'] || Q.inputs['action'])) {
          p.jumping = false;
          this.entity.trigger('jumped', this.entity);
          if(p.vy < p.jumpSpeed / 3) {
            p.vy = p.jumpSpeed / 3;
